...
> **ReimburseAI: AI Reimbursement Bill Scanner** | Python, FastAPI, Azure AI Document Intelligence, Azure OpenAI, Azure Blob Storage, App Service, GitHub Actions
> - Built an AI pipeline that extracts vendor, date and amount from bill photos/PDFs and auto-categorises them into flexi-benefit heads using GPT with structured JSON output and a rule-based fallback.
> - Implemented policy and fraud checks (SHA-256 duplicate detection, near-duplicate matching, monthly category limits, date validation), surfacing warnings in an HR approval dashboard.
> - Built a Smart Benefit Rebalancer from real App Store user feedback, letting employees move unused flexi-benefit limits between categories with HR approval, a fixed total budget, held funds for pending requests, re-validation at approval and a full audit trail.
> - Deployed on Azure App Service with private Blob storage; set up CI/CD in GitHub Actions with 32 automated tests, gated deployment and a post-deploy health check.
